function Data = prepareFittingData2(obsDir, varargin)
% Load and clean fitting data, output as struct

% change: load all data, dont omit unmatched samples
...
% NOT AVAILABLE (samples have never been taken)
...
% MSM77 cruise
%~~~~~~~~~~~~
OMObsFile = '2_MSM77_Chl_a_und_POC_Übersicht_EN0719_Kopie.csv';

% correct this later: use correct file path to MSM77 OM data file
% dat = readtable(fullfile(obsDir,OMObsFile), 'Format', 'auto');
% fileparts(which(OMObsFile))

dat = readtable(fullfile(fileparts(which(OMObsFile)), OMObsFile), 'Format', 'auto'); 


% reformat lat/long columns -- use +/- insteqad of E/W, and use decimal-degrees
% instead of degree-minutes
west = contains(dat.Longitude, 'W');
dat.Longitude = strrep(dat.Longitude,"' E","");
dat.Longitude = strrep(dat.Longitude,"' W","");
dat.Longitude = strrep(dat.Longitude,',','.');
%dat.Longitude = strrep(dat.Longitude,' ','.');
degr = split(dat.Longitude, 'ｰ '); % convert degree-minutes/seconds into degree-decimal
minute = str2double(degr(:,2));
%second = str2double(degr(:,3));
degr = str2double(degr(:,1));
lon = degr + minute ./ 60; % + second ./ 3600
...
dat.Latitude = strrep(dat.Latitude, "' N", "");
dat.Latitude = strrep(dat.Latitude, ',', '.'); 
degr = split(dat.Latitude, 'ｰ ');
minute = str2double(degr(:,2));
% second = str2double(degr(:,3));
degr = str2double(degr(:,1));
dat.Latitude = degr + minute ./ 60; % + second ./ 3600
% reformat event labelling...
dat.Properties.VariableNames{'cast'} = 'Label';
x = regexp(dat.Label, '_');
for i = 1:length(x)
    dat.Label{i}(x{i}(1)) = '/';
end
...
x_2 = x_(:,2);
for i = 1:length(x_1)
    x_1{i} = sprintf('%03d', str2double(x_1{i}));
    x_2{i} = sprintf('%02d', str2double(x_2{i}));
end
x_ = join([x_1, x_2], '-');
dat.Label = join([x(:,1), x_], '/');

dat.Properties.VariableNames{'date'} = 'Date';
%fix years from 18 A.D. to 2018...
dat.Date = dat.Date + years(2000); 
% timestamp column is also weird (all dates lie in 2022), but this comes from the original data

% include extra timing columns
dat.t = datenum(dat.Date);
[dat.Year,~] = datevec(dat.t);
dat.Yearday = yearday(dat.t);
dat.Properties.VariableNames{'Timestamp_UTC_'} = 'Time';
% dat.Time = timeofday(dat.Time');  % , 'Format', 'HH:mm', 'Format','mm:ss'
dat.Time = cellstr(datestr(dat.Time,'HH:MM'));

% rename data columns, reformat values from decimal-comma-strings to
% something more useful
dat.Properties.VariableNames{'ChlaFluor_'} = 'Chla';
dat.Chla = str2double(strrep(dat.Chla, ',', '.')); 
dat.POC = str2double(strrep(dat.POC, ',', '.')); 

% rename depth column
dat.Properties.VariableNames{'Tiefe'} = 'Depth';

% filter out data columns
% we dont have PON here, so add NAN column
dat.PON = repelem(NaN, height(dat), 1); 
dat = dat(:,{'Label','t','Date','Time','Year','Yearday','Latitude','Longitude','Depth','Chla','POC','PON'});
dat.Properties.VariableNames{'Chla'} = 'chl_a';
% Convert POM units from (mu g / L) to (m mol / m^3)
dat.PON = dat.PON ./ 14; 
...
% merge with PS99 & PS107 & PS114 data
% dat_OM.Time = cell(height(dat_OM),1);
% dat_OM = movevars(dat_OM, 'Time', 'After', 'Date');
dat_OM = [dat_OM; dat];
...
dat_size.BioVolDensitySE = abs(scaleFactor .* dat_size.cellVolume) .* dat_size.cellDensitySE;
...
keepVars = [{'scenario', 'season', 'regime', 'trophicLevel', 'ESD', 'cellVolume', ...
    'cellDensity', 'cellDensitySE', 'BioVolDensity', 'BioVolDensitySE'}, strcat('Ndensity_', NsizeEqs), 'Ndensity'];
...
scenarioInfo.S1.dateLast = datetime(2017,8,19);
scenarioInfo.S2.dateFirst = datetime(2016,6,23);
scenarioInfo.S2.dateLast = datetime(2017,8,19);
scenarioInfo.S3.dateFirst = datetime(2018,9,16);
scenarioInfo.S3.dateLast = datetime(2018,10,12);
scenarioInfo.S4.dateFirst = datetime(2018,9,16);
scenarioInfo.S4.dateLast = datetime(2018,10,12);
scenarioInfo.S1.depth = [5 43];
scenarioInfo.S2.depth = [5 43];
scenarioInfo.S3.depth = [5 34];
